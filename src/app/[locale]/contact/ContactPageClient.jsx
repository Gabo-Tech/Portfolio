        ...
        >
          ...
          >
            ...
            <input
              ...
              onChange={handleInputChange}
            />
            <p className="text-sm text-stone-400 min-[500px]:text-base md:text-lg 2xl:text-xl">{t("regards")}</p>
            <motion.button
              type="submit"
              whileHover={
                !isFormValid || reducedMotion
                  ? undefined
                  : { scale: 1.02, boxShadow: "0px 0px 8px rgba(0, 0, 0, 0.8)" }
              }
              whileTap={!isFormValid || reducedMotion ? undefined : { scale: 0.99 }}
              disabled={!isFormValid}
              className={`min-h-12 w-full max-w-full cursor-pointer items-center justify-center gap-2 rounded-xl px-4 py-3.5 font-display text-base font-semibold [touch-action:manipulation] sm:min-h-14 sm:gap-3 sm:px-5 sm:py-4 sm:text-lg md:text-xl 2xl:py-5 2xl:text-2xl ${
                success
                  ? "bg-emerald-500/90 text-white"
                  : error
                    ? "bg-red-600/90 text-white"
                    : "bg-sky-500 text-white border border-sky-400/40"
              } ${!isFormValid ? "cursor-not-allowed opacity-50" : ""} flex flex-wrap sm:flex-nowrap`}
            >
              {success ? (
                <>
                  {t("sentSuccess")}
